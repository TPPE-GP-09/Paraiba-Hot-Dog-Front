...
export default function AnotarPedidos() {
  ...
  const pedidosAbertosNaoPagos = useMemo(
    () => pedidosAbertos.filter((pedidoAberto) => pedidoAberto.status === 'aberto'),
    [pedidosAbertos],
  )
      ...
      <div className="mx-auto grid max-w-[1900px] lg:grid-cols-[230px_minmax(0,1fr)_360px]">
        <aside className="hidden min-h-[calc(100vh-4rem)] border-r border-slate-200 bg-[#f5f7fb] px-4 py-8 lg:block">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="flex h-7 min-w-7 items-center justify-center rounded-full bg-amarelo px-2 text-xs font-black text-preto-v1">{pedidosAbertosNaoPagos.length}</span>
          </div>
          ...
          <div className="mt-3 max-h-[calc(100vh-13rem)] space-y-2 overflow-y-auto pr-1">
            {pedidosAbertosNaoPagos.length === 0 ? (
            ...
            ) : pedidosAbertosNaoPagos.map((pedidoAberto) => (
        ...
        <main className="min-w-0 px-4 py-6 sm:px-6 lg:px-7 lg:py-8">
          <section className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:hidden">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Pedidos abertos</p>
                <p className="mt-1 text-[10px] text-slate-400">Ainda não pagos</p>
              </div>
              <span className="flex h-7 min-w-7 items-center justify-center rounded-full bg-amarelo px-2 text-xs font-black text-preto-v1">{pedidosAbertosNaoPagos.length}</span>
            </div>

            <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
              <button type="button" onClick={() => abrirPedidoExistente(null)} className={`flex min-w-[13rem] items-center gap-3 rounded-xl border p-3 text-left transition ${!pedidoAbertoId ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-slate-200 bg-white'}`}>
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-preto-v1 text-white"><Plus size={17} /></span>
                <span><span className="block text-xs font-black uppercase">Novo pedido</span><span className="text-[10px] text-slate-400">Iniciar comanda</span></span>
              </button>
              {pedidosAbertosNaoPagos.map((pedidoAberto) => (
                <button key={pedidoAberto.id} type="button" onClick={() => abrirPedidoExistente(pedidoAberto.id)} className={`min-w-[13rem] rounded-xl border p-3 text-left transition ${pedidoAbertoId === pedidoAberto.id ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-200 bg-white'}`}>
                  <div className="flex items-start justify-between gap-2"><span className="text-xs font-black">Pedido #{pedidoAberto.id}</span><span className="rounded-full bg-amber-100 px-2 py-0.5 text-[8px] font-black uppercase text-amber-700">Aberto</span></div>
                  <p className="mt-2 truncate text-xs font-semibold text-slate-600">{pedidoAberto.nome_comanda}</p>
                  <div className="mt-2 flex items-center justify-between text-[10px] text-slate-400"><span>{pedidoAberto.itens.reduce((total, item) => total + item.quantidade, 0)} itens</span><strong className="text-slate-700">{moeda(Number(pedidoAberto.total))}</strong></div>
                </button>
              ))}
              {pedidosAbertosNaoPagos.length === 0 && (
                <div className="flex min-w-[13rem] items-center justify-center rounded-xl border border-dashed border-slate-300 px-4 py-5 text-center text-xs font-semibold text-slate-500">
